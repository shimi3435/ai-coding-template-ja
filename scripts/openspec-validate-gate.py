"""OpenSpec validate の opt-in ゲート（task openspec:validate の実体）。
...
（fail-open）ため、CLI 実行前に preflight で proposal.md / tasks.md の在席を検査する。
change の列挙・欠落判定は doctor.py の list_change_dirs / broken_change_dirs を
単一の正として共有する。engine 必須（不在は導入案内＋非ゼロ終了）。
"""

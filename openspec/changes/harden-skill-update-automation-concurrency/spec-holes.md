...
| 2 | 境界値 | 該当 |対象entry 0 / 1 / 2件、issue 0 / 1 / 複数root、cleanup failed ref 0 / 1 / 複数 | 1:recoveryはexact candidate scopeの2 keyだけ処理。stable no-opは既存detection scope規則とcleanup current setも処理。identity cardinality不正はwrite禁止 |
...
| 7 | 冪等性・再実行 | 該当 |ready-recovered同run失敗、Issue create / commentless root回復後の中断、stale rediscovery、後続no-op retry、統合transition応答消失 | 1:stable ready / passed状態から同candidate keyとhealthy detection / cleanup current setを再計算。`created` / `recovered` / `updated`を正常化し、root continuation budget消費後はroot entryを再送しない。fresh rediscovery後の解消済みentryはno-op |
| 8 | 時刻・タイムゾーン | 非該当 |reconciliation eligibilityは経過時間を使わない | 2:fresh exact stateだけを根拠にする |
| 9 | 文字列 | 該当 |candidate digest / scope / failure key / cleanup ref表現差 | 1:canonical digest、既存detection scope predicate、canonical cleanup refを使用。candidate-scoped permissionは由来operation不明のため保持 |
...
| 12 | 状態遷移の未定義パス | 該当 |PRはreadyだがvalidation非passed、branch / root不一致、permission / cleanup / updater entry混在、cleanup evidence未取得、Issue absent / commentless / journal済み | 1:readyかつpassedのexact candidateだけ処理。recoveryはnarrow、healthy no-opはdetectionとcleanupを含む単一transition。cleanup evidence未取得時と対象外scopeは保持 |
...
| ready-recovered issue reconciliation / later no-op retry | public lifecycle / workflow test | `recovery/lifecycle.test.ts`, `finalize/finalize.test.ts`, `finalize/workflow.test.ts` | recoveryは同candidateの2 failure keyだけ解消。stable no-opは既存detection scopeとcleanup current setを同一transitionでreconcile。cleanup evidence未取得、candidate permission、partial read、identity conflictは保持またはworkflow red |
| stable no-op current-state reconciliation | public reconciliation test | `finalize/finalize.test.ts`, `finalize/detection-failure.test.ts` | cleanup passed、updater recovery、cleanup ref A→B、recovery narrow、cleanup evidence未取得、response-loss retryの6境界を確認 |

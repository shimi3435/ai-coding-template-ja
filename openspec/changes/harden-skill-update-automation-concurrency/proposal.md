...
- recovered `pr-ready`は、既存`publish-finalize` jobで同じcandidateの`validation-failed` / `recovery-required` tracking entryだけを冪等に解消する。後続のstable ready / passed no-opは同じ2 entryに加え、healthy detectionが否定した既存detection entryを既存scope規則どおり解消し、cleanupのpassed / failed evidenceをcurrent setへreconcileする。cleanup evidenceがなければ既存cleanup entryを保持する。全observationsとresolved keysは1回のcanonical Issue transitionへ統合する。既存Issueなしでcurrent cleanup failureを記録するcreate、commentless Issue rootのexact initial entry回復後にstale candidate failureを解消する二段reconciliationも正常完了として扱う。root appendをfresh確認した後のrediscoveryがstale commentless projectionへ回帰した場合、root entryを再appendせずfail closedにする。`recover` jobへ`issues: write`は追加しない。

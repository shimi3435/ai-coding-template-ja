...
- [x] 7. Codex 再レビュー反映: gate 実体を `scripts/openspec-validate-gate.py` に切り出し（preflight 判定は doctor のヘルパーを単一の正として共有）、gate の回帰テストを openspec / task 実物に依存しない常時実行テストへ置き換える

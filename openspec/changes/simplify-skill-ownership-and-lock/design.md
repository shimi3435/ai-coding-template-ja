...
取得先はgithub.com上の公開repositoryに限定する。共通apiJsonで全 `gh api` 引数へ
`--hostname github.com` を指定し、GH_HOST等の既定host設定に依存しない。
createGhRunnerは渡された引数を実行し、環境変数の一括削除やhost選択interfaceは追加しない。

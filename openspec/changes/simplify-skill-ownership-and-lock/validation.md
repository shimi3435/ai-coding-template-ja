...
V3のGhRunner seamで全API引数に `--hostname github.com` が一度だけ含まれることを検査する。
GH_HOST=enterprise.invalidを子processに設定し、branch / tag / commitの取得成功とcompareを含む呼出を検査する。
実ghの固定commit read-only取得も同じ環境条件でfresh実行する。実remoteは通常CIへ含めない。
実行順・完了状態はtasks.mdのtask 9に従う。

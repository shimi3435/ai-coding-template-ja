...
## Correction cycle 3の再監査

Safe retirement guidanceの12分類を再適用した。未解決判断なし。

| # | 分類 | 判断 | 穴と解決 |
| --- | --- | --- | --- |
| 1 | 空・ゼロ長・None | 該当 | amendment欠落は回帰検査失敗、PR番号不明はclose保留とする |
| 2 | 境界値 | 非該当 | 数量境界を変更しない |
| 3 | 重複・衝突 | 該当 | Accepted本文と現行決定の衝突を冒頭の部分改訂で解消する。PRは既存headを確認し重複作成しない |
| 4 | 順序 | 該当 | 修正・focused検証・review→Draft PR→実番号反映→最新check/verifier→closeとする |
| 5 | 型・形式不正 | 該当 | 実番号を（PR #N）の固定形式へ反映し、Issue番号やplaceholderを代用しない |
| 6 | エラー経路 | 該当 | PR作成/確認失敗時は実在状態を再取得し、closeを保留する |
| 7 | 冪等性・再実行 | 該当 | 同一headの既存PRを再利用し、retrospectiveの既存1行を更新する |
| 8 | 時刻・タイムゾーン | 非該当 | retrospectiveの日付を変更せず発見経路と件数を更新する |
| 9 | 文字列 | 該当 | 旧本文を保持し、冒頭とリンクだけを改訂する。本文の抹消で解消しない |
| 10 | 数値 | 該当 | 先行1件＋新規2件＝3件、review=3、merge後=0とする |
| 11 | 巨大入力・リソース枯渇 | 非該当 | 新規取得処理や入力形式を実装しない |
| 12 | 状態遷移の未定義パス | 該当 | PRなしの最終closeを認めず、作成済みPRはdraftのまま保持しmergeしない |

検証対応: 1/3/9は既存documentation contractのADR回帰検査と本文不変probe、4/5/6/7/10/12はtasksの順序・ghによるPR identity確認・retrospective正規表現検査・独立review/verifierで確認する。実GitHub API障害の注入は対象外・未検証であり、障害発生時は重複作成せず停止する。履歴・change directoryを通常CIの入力へ追加しない。

...
## Correction cycle 3（利用者合意済み）

head 577f1b1へのadversarial re-reviewでADR-0011のAcceptedな#64任意提供方針と、PR番号なしのcloseが指摘された。grillingの合意により、ADRは部分改訂を冒頭へ追記し当時の本文を保持する。対象は#64のdownstream optional/opt-in/pruneと#64へのplugin更新PR統合案であり、他の責務境界は一括失効させない。
Draft PR作成を今回の範囲に含め、実際のPR番号をふりかえりに反映してから最終closeする。新しい2件は別のcontract defectであり、先行1件と合わせて計3件・review=3とする。PR作成は承認済みだがmerge・旧automationのGitHub resource操作は対象外である。

...
設けず、次の追加を実質的な拡張（material expansion）として停止・再計画する。
...
| 2 | 境界値 | 該当 | 何件から過剰か固定値がない | 1: 件数でなく実質的な拡張の条件を固定する |
...
| 9 | 文字列 | 該当 | 用語揺れで分類が崩れる | 1: `CONTEXT.md` の5用語へ正規化する |
| 10 | 数値 | 非該当 | token / 行数の固定数値を契約にしない | 2: 自動 accounting は対象外 |
| 11 | 巨大入力・リソース枯渇 | 該当 | 実行中に計画・証跡が膨張する | 1: 実質的な拡張で停止・再計画する |

...
`.github/workflows/skill-update-prs.yml` は vendored skill 更新を一つの managed PR にまとめる。
既定は無効。Repository Actions variable `SKILLS_AUTO_UPDATE` を文字列 `true` にした場合だけ、毎週の
schedule が動く。手動実行は Actions の `workflow_dispatch` から行い、入力は boolean

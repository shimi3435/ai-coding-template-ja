...
- **At Wave 17 / Plan 03-23 Task 1:** Run `task check`, generate the independent review, parse authoritative YAML frontmatter for `status=clean` plus nested findings critical/warning/info/total all integer zero, and require no canonical Critical/Blocker/Warning/Info finding record in the body.
- **At Wave 17 / Plan 03-23 Task 2:** Generate the independent security report, parse authoritative YAML frontmatter for `status=verified`, ASVS level 1, and `threats_open=0`, then parse the canonical Threat Register and require every row closed, including each high-severity row when a Severity column is present.
- **After Wave 17 / Plan 03-23 Task 3:** Run `task openspec:validate`, `task check`, and `git diff --check`; parse final verification frontmatter for passed, exact 10/10, behavior_unverified 0, overrides_applied 0, optional gaps/deferred/overrides absent-or-empty, and canonical re_verification gaps_remaining/regressions empty; parse the Requirements Traceability table and require the exact HND-03 Complete tuple plus exact unchanged values for all six unrelated rows. This is the final sampling gate across all 23 plan must-have groups.
...
- [ ] Generate 03-23 review/security/verifier reports independently and gate them through exact YAML frontmatter parsing, not prose searches.
...
- Any Wave 16 mutation/lock-contention failure blocks independent exit evidence. A hash-only check without the shared lock lifetime, a lock on a different inode/scope, an unlocked fallback, retry, candidate installation, or changed concurrent bytes is a hard failure.
- Any Wave 17 missing/unparsable report frontmatter, nonzero review/security/verifier count, non-empty gaps/deferred list, non-exact HND-03 object/traceability tuple, or unrelated traceability-row change blocks final sign-off regardless of prose elsewhere.
...
- [x] Plans 03-19 through 03-23 contribute all seven tasks to the Wave 15/16/17 verification map.
...
- [x] Wave 15 samples all three independent implementation gaps, Wave 16 proves the shared writer lock and fixed injected race, and Wave 17 parses every canonical exit artifact plus exact traceability before final project gates.

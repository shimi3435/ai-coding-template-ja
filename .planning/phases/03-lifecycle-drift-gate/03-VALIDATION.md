...
- **At Wave 17 / Plan 03-23 Task 1:** Run `task check`, generate the independent review, then parse only its authoritative YAML frontmatter and require `status=clean` plus nested findings critical/warning/total/unresolved all integer zero.
- **At Wave 17 / Plan 03-23 Task 2:** Generate the independent security report, then parse only its authoritative YAML frontmatter and require `status=verified`, ASVS level 1, `threats_open=0`, and `high_threats_open=0`.
- **After Wave 17 / Plan 03-23 Task 3:** Run `task openspec:validate`, `task check`, and `git diff --check`; parse final verification frontmatter for passed, exact 10/10, behavior_unverified 0, overrides_applied 0, `gaps=[]`, `deferred=[]`, and exact satisfied HND-03/HARD-R2; parse the Requirements Traceability table and require the exact HND-03 Complete tuple plus exact unchanged values for all six unrelated rows. This is the final sampling gate across all 23 plan must-have groups.
...
| 03-23-01 | 23 | 17 | HND-03 / HARD-R2 | T-03-23-SELF, T-03-23-ORDER | Independent review frontmatter is authoritatively clean with critical/warning/total/unresolved all zero | independent report + exact YAML parse | `uv run python -c "from pathlib import Path; import yaml; text=Path('.planning/phases/03-lifecycle-drift-gate/03-REVIEW.md').read_text(encoding='utf-8'); parts=text.split('---', 2); assert len(parts)==3; fm=yaml.safe_load(parts[1]); findings=fm.get('findings'); assert fm.get('status')=='clean'; assert isinstance(findings, dict); assert {key: findings.get(key) for key in ('critical','warning','total','unresolved')}=={'critical':0,'warning':0,'total':0,'unresolved':0}"` | ✅ | ⬜ pending |
| 03-23-02 | 23 | 17 | HND-03 / HARD-R2 | T-03-23-SELF, T-03-23-ORDER | Independent security frontmatter is verified ASVS L1 with zero open threats and zero open high threats | independent threat audit + exact YAML parse | `uv run python -c "from pathlib import Path; import yaml; text=Path('.planning/phases/03-lifecycle-drift-gate/03-SECURITY.md').read_text(encoding='utf-8'); parts=text.split('---', 2); assert len(parts)==3; fm=yaml.safe_load(parts[1]); assert {key: fm.get(key) for key in ('status','asvs_level','threats_open','high_threats_open')}=={'status':'verified','asvs_level':1,'threats_open':0,'high_threats_open':0}"` | ❌ created by 03-23-02 | ⬜ pending |
| 03-23-03 | 23 | 17 | HND-03 / HARD-R2 | T-03-23-ORDER, T-03-23-OVERRIDE | Final verification frontmatter is passed 10/10 with zero unverified/overrides and empty gaps/deferred; the exact HND-03 traceability row alone advances | independent verifier + exact YAML/table parsers + project gates | `task openspec:validate && task check && git diff --check && uv run python -c "from pathlib import Path; import yaml; verification=Path('.planning/phases/03-lifecycle-drift-gate/03-VERIFICATION.md').read_text(encoding='utf-8'); parts=verification.split('---', 2); assert len(parts)==3; fm=yaml.safe_load(parts[1]); assert fm.get('status')=='passed'; assert fm.get('score')=='10/10 must-haves verified'; assert fm.get('behavior_unverified')==0; assert fm.get('overrides_applied')==0; assert fm.get('gaps')==[]; assert fm.get('deferred')==[]; assert fm.get('requirements')==[{'id':'HND-03','canonical_handle':'HARD-R2','status':'satisfied'}]; requirements=Path('.planning/REQUIREMENTS.md').read_text(encoding='utf-8'); section=requirements.split('## Traceability',1)[1].split('**Coverage:**',1)[0]; rows=[tuple(cell.strip() for cell in line.strip().strip('|').split('|')) for line in section.splitlines() if line.lstrip().startswith('| HND-')]; expected=[('HND-01','HARD-R1','Phase 1','Complete'),('HND-02','HARD-R1','Phase 2','Pending'),('HND-03','HARD-R2','Phase 3','Complete'),('HND-04','HARD-R3','Phase 4','Pending'),('HND-05','HARD-R4','Phase 5','Pending'),('HND-06','HARD-R5','Phase 6','Pending'),('HND-07','HARD-R6','Phase 6','Pending')]; assert rows==expected"` | ✅ | ⬜ pending |

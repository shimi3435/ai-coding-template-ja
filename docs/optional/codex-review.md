...
- レビュー: `/codex:review`（読み取り専用）。PR 前のセルフチェックに使う。
  **focus（追加のレビュー観点テキスト）は非対応**（plugin v1.0.5 で確認・2026-07-10）。
- 観点付きレビューは `/codex:adversarial-review` を使う（読み取り専用のまま、実装方針・
  設計判断・前提に挑戦する位置づけのレビュー。focus テキストを渡せる）。
- Codex クロスレビューは PR 前に限らず、任意のレビューチェックポイント（実装途中・設計確認・
  リファクタ後など）で使える（人起点）。走る保証があるのは常時実行する `self-review` で、
  Codex はそこへの上乗せ。

...
Skill更新PR自動化の提供は終了した。既存環境は[停止・撤去手順](../guide.md#7-skillの手動更新と旧自動化の撤去)に従い、
workflow無効化とrun停止を確認してから移行する。repository内の撤去処理でGitHub上のresourceを自動削除しない。
利用者変更や共有credentialは保持し、所有・状態が不明なresourceを推測でcleanupしない。
手動更新は別の作業コピーでpreview、明示apply、offline検証、通常PRの順に行う。
取得前上限、integrity、承認済みlegal hash、履歴変更時の停止を維持し、取得物のscriptを実行しない。
